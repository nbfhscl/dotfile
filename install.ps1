...
function Deploy-NeovimConfig {
    ...
    try {
        Copy-Item -Path "$nvimSource\*" -Destination $nvimTargetLinux -Recurse -Force -ErrorAction Stop
        ...
        if (Test-Path $lazyLockFile) {
            ...
            Write-Warning "lazy-lock.json not found (will be generated by lazy.nvim)"
        }
        ...
        Copy-Item -Path "$nvimSource\*" -Destination $nvimTargetWindows -Recurse -Force -ErrorAction Stop
        ...
        try {
            ...
            Write-Warning "Could not set XDG_CONFIG_HOME permanently: $_"
...
function Verify-NeovimConfig {
    ...
    if (-not (Get-Command nvim -ErrorAction SilentlyContinue)) {
        Write-Warning "Neovim is not installed or not in PATH"
    ...
    if ($env:XDG_CONFIG_HOME) {
        ...
        Write-Warning "XDG_CONFIG_HOME not set in current session"
    ...
    foreach ($path in $configPaths) {
        if (Test-Path $path) {
            ...
            if (Test-Path $initLua) {
                ...
                Write-Warning "  init.lua not found"
            }
            ...
            Write-Warning "Config directory not found: $path"
        }
    }
    ...
    try {
        ...
        Write-Warning "Could not query Neovim for config path: $_"
    }
